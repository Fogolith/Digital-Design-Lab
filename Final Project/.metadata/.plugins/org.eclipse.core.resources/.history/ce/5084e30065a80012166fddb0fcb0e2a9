...
//	private static final String PREFS_TRACKASSCROLL = "trackasscroll";
	...
	public static boolean hideMouseButtons = false;
	public static boolean twoTouchRightClick;

	public static void init(Context con) {
		if (prefs == null) {
			prefs = con.getSharedPreferences(PREFS_FILENAME, Context.MODE_PRIVATE);
			charmap = KeyCharacterMap.load(KeyCharacterMap.VIRTUAL_KEYBOARD); // DEPRICATED .BUILT_IN_KEYBOARD
	...
	private static void writeRecentIPsToSettings() {
		...
		for (int i = 0; i < MAX_SAVED_HOSTS; ++i) {
			...
			edit.putString(PREFS_RECENT_IP_PREFIX + Integer.valueOf(i).toString(), s);
		}
		edit.commit();
	}
	...
	private static void populateRecentIPs() {
		savedHosts.clear();
		for (int i = 0; i < MAX_SAVED_HOSTS; ++i) {
			String host = prefs.getString(PREFS_RECENT_IP_PREFIX + Integer.valueOf(i).toString(), null);
			if (host != null) {
				savedHosts.add(host);
			}
		}
