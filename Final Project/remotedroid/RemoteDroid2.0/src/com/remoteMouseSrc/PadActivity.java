...
import com.remoteMouse.R;
import com.remoteMouseSrc.PadActivity.PanelConfig.PanelItem;
import com.remoteMouseSrc.BluetoothServiceHelper; 
...
public class PadActivity extends Activity implements BluetoothServiceHelper.EEGHandler {
	//
    ...
    private BluetoothServiceHelper mBluetoothHelper;
			...
			public PanelItem(String config)
			{
				...
				for (int i = 0; i < config.length(); i++)
				{
					...
					else
					{
						if (chr.equals(":"))
						{
							name = current.trim().toLowerCase(new Locale("US")); 
							current = "";
						}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (this.lock == null) {
			...
			//this.lock = manager.newWakeLock(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON, this
			//		.getString(R.string.app_name));
		...
		mBluetoothHelper = new BluetoothServiceHelper(this, this, null, this.getString(R.string.BluetoothAddress) );//
		mBluetoothHelper.startService();
	}
			...
			*/
			...
			if (key == 0)
				for (int z = 0; z < 1024; z++)
				{
					...
					{
						if (Character.valueOf(Character.toChars(Settings.charmap
			...
			{	
				if(isCtrl){
					...
					args[0] = 0; /* key down */
					args[1] = 57;// (int)c;
					args[2] = Character.valueOf((char)0).toString();
					OSCMessage msg = new OSCMessage("/keyboard", args);
	...
	private void drawSoftOff() {
      ...
      public void onBTDisconnected() {
          Log.d("Lab8", "Bluetooth disconnected");
      }
