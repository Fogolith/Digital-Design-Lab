...
public class BluetoothServiceHelper {
    ...
    public static final int MSG_RAW_DATA_EMG = 29;		
    public static final int MSG_DATA_NOISE = 30;		// Signal quality
    public static final int MSG_RAW_DATA_ACC = 33;		
	...
	public BluetoothServiceHelper(Context c, EEGHandler handler, String username, String btAddress) {
		mContext = c;
		mHandler = handler;
		mBTAdress = btAddress;
		mUsername = username;
    ...
    private Handler mDataServiceHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_RAW_DATA_EMG:
            	...
            	index = msg.getData().getInt("index");
            	mHandler.onRawEMGData(rawData, index);
            	break;
            case MSG_RAW_DATA_ACC:
            	Bundle b2 = msg.getData();
            	size = b2.getInt("size");
            	float[][] rawACCData = new float[size][];
            	for (int i = 0; i < size; i++) {
            		rawACCData[i] = b2.getFloatArray("chan"+i);
            	}
            	index = msg.getData().getInt("index");
            	mHandler.onRawACCData(rawACCData, index);
            case MSG_DATA_NOISE:
