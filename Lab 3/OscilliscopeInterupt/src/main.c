...
void TIMERInit() {
	...
	int TimerInterval = (SystemCoreClock/1000 - 1); //1MS
...
void PIOINT2_IRQHandler(void)
{
	...
	LPC_GPIO2->IC |= (0x1<<1);
	period = timer32_0_counter - lastLowIntTime;
...
void TIMER32_0_IRQHandler(void)
{
	...
	if((timer32_0_counter % 60000) > 30000)
	{
		dutyCycle = 1;
	}
	else
	...
	// 25% duty cycle
	if((timer32_0_counter - lastLowIntTime) > period/4)
	{
		SetPin( LED_PORT, LED_BIT, 0 ^ dutyCycle );
	}
	else
	{
		SetPin( LED_PORT, LED_BIT, 1 ^ dutyCycle );
	}
}
