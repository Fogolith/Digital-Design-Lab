...
public class HeadlinesFragment extends ListFragment implements SensorEventListener {
    ...
    // TODO Auto-generated method stub
        ...
        if(event.sensor.getType()==Sensor.TYPE_LINEAR_ACCELERATION){
             ...
             if (Math.abs(x) < aThreshold && Math.abs(y) < aThreshold) {
                     return;
             }
    
             if (Math.abs(x) > Math.abs(y)) {
            	 //mTextView.setText("Large X"); 
            	 //Toast.makeText(getActivity(), "Large X",
            	 ...
            	 //mTextView.setText("Large Y"); 
                     //mCallback.onArticleSelected(1);
            	 Toast.makeText(getActivity(), "Large Y",
                         Toast.LENGTH_SHORT).show();
